<div id="content" class="bg-gray-200 h-[100vh]">
    <?php
session_start();
...
if (isset($_POST['submit'])) {
    ...
    // Check if the academic year already exists in the database
    $checkQuery = "SELECT * FROM accademicyear WHERE year = '$year'";
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        // Academic year does not exist, proceed with insertion
        // Academic year does not exist, proceed with insertion
        $query = "INSERT INTO `accademicyear`(`year`,`status`, `date`) VALUES ('$year','$status', '$date')";
        ...
        if ($insert) {
            ...
            // Unable to register academic year
            // Unable to register academic year
            echo "<script>
                    alert('Unable to register academic year');
                    window.location.href = 'accadmicYear.php';
                </script>";
        }
    }
                ...
                <div class="text-right">
                    <form action="" method="post">
                        ...
                        <input type="submit" value="Add"
                            class="bg-white h-8  w-12 rounded-sm text-gray-600 shadow-sm text-center  ml-2 "
                            name="submit">
                    ...
                    ?>
                        <tbody class="text-[13px] text-gray-600">
                            <tr class="even:bg-[#e9e3ff] h-10">
                                ...
                                <td>
                                    <a href="accadmicYear.php?id=<?php echo $row['id'] ?>">
                                        <div
                                            class="bg-green-500 flex items-center justify-center text-white w-14 h-5 text-center rounded-sm">
                                            <?php echo $row["status"] ?>
                                        </div>
                                    </a>
                                </td>
                                ...
                                <td>
                                    <div class="flex gap-[2px]">
                                        <a href="teacher_reg.php?id=<?php echo $row['id'] ?>">
                                            <div
                                                class="bg-green-500 flex items-center justify-center text-white w-6 h-5 text-center rounded-sm">
                                                <button><i class="fa fa-edit"></i></button>
                                            </div>
                                        </a>
                                        <a href="teachers_reg.php?delete=<?php echo $row['id'] ?>">
                                            <div
                                                class="bg-red-600 flex items-center justify-center text-white w-6 h-5 text-center rounded-sm">
